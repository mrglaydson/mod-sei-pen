...
class MdAbcExemploIntegracao extends SeiIntegracao{
  ...
  public function montarBotaoControleProcessos(){
    ...
    */
    ...
    if (SessaoSEI::getInstance()->verificarPermissao('md_abc_andamento_lancar')) {
      $arrBotoes[] = '<a href="#" onclick="return acaoControleProcessos(\''  . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_abc_andamento_lancar&acao_origem=procedimento_controlar&acao_retorno=procedimento_controlar') . '\', true, false);" tabindex="' . PaginaSEI::getInstance()->getProxTabBarraComandosSuperior() . '" class="botaoSEI"><img class="infraCorBarraSistema" src="modulos/abc/exemplo/imagens/abc_grande.png" alt="Botão Controle de Processos ABC" title="Botão Controle de Processos ABC" /></a>';
    }
  ...
  public function montarIconeControleProcessos($arrObjProcedimentoAPI){
    ...
    foreach($arrObjProcedimentoAPI as $objProcedimentoAPI) {
      $arrIcones[$objProcedimentoAPI->getIdProcedimento()][] = '<a href="javascript:void(0);" '.PaginaSEI::montarTitleTooltip('Ícone Controle de Processos ABC','Módulo ABC').'><img src="modulos/abc/exemplo/imagens/abc_pequeno.png" class="imagemStatus" /></a>';
    }
  ...
  public function montarIconeAcompanhamentoEspecial($arrObjProcedimentoAPI){
    ...
    foreach($arrObjProcedimentoAPI as $objProcedimentoAPI) {
      $arrIcones[$objProcedimentoAPI->getIdProcedimento()][] = '<a href="javascript:void(0);" '.PaginaSEI::montarTitleTooltip('Ícone Acompanhamento Especial ABC','Módulo ABC').'><img src="modulos/abc/exemplo/imagens/abc_pequeno.png" class="imagemStatus" /></a>';
    }
  ...
  public function montarIconeProcesso(ProcedimentoAPI $objProcedimentoAPI){
    ...
    if (SessaoSEI::getInstance()->verificarPermissao('md_abc_processo_processar') && $objProcedimentoAPI->getCodigoAcesso() > 0 && $objProcedimentoAPI->getSinAberto()=='S') {
      ...
      $objArvoreAcaoItemAPI->setIcone('modulos/abc/exemplo/imagens/abc_pequeno.png');
  ...
  public function alterarIconeArvoreDocumento(ProcedimentoAPI $objProcedimentoAPI, $arrObjDocumentoAPI){
    ...
    foreach ($arrObjDocumentoAPI as $objDocumentoAPI) {
      if ($objDocumentoAPI->getIdSerie()==$numIdSerieAbc) {
        $arrIcones[$objDocumentoAPI->getIdDocumento()] = 'modulos/abc/exemplo/imagens/abc_pequeno.png';
      }
    }
  ...
  public function adicionarElementoMenu(){
    return '<div style="text-align:center;"><img src="modulos/abc/exemplo/imagens/abc_menu.png" alt="Imagem Menu" title="Imagem Menu" /></div>';
  }
  ...
  public function montarAcaoControleAcessoExterno($arrObjAcessoExternoAPI){
    ...
    foreach($arrObjAcessoExternoAPI as $objAcessoExternoAPI) {
      $arrIcones[$objAcessoExternoAPI->getIdAcessoExterno()][] = '<a href="javascript:void(0);" '.PaginaSEI::montarTitleTooltip('Ícone Ação Acesso Externo ABC','Módulo ABC').'><img src="modulos/abc/exemplo/imagens/abc_pequeno.png" class="imagemStatus" /></a>';
    }
  ...
  public function montarAcaoDocumentoAcessoExternoAutorizado($arrObjDocumentoAPI){
    $arrIcones = array();
    foreach($arrObjDocumentoAPI as $objDocumentoAPI) {
      $arrIcones[$objDocumentoAPI->getIdDocumento()][] = '<a href="javascript:void(0);" '.PaginaSEI::montarTitleTooltip('Ícone Ação Documento Acesso Externo Autorizado ABC','Módulo ABC').'><img src="modulos/abc/exemplo/imagens/abc_pequeno.png" class="imagemStatus" /></a>';
    }
    return $arrIcones;
  }
  ...
  public function montarAcaoProcessoAnexadoAcessoExternoAutorizado($arrObjProcedimentoAPI){
    $arrIcones = array();
    foreach($arrObjProcedimentoAPI as $objProcedimentoAPI) {
      $arrIcones[$objProcedimentoAPI->getIdProcedimento()][] = '<a href="javascript:void(0);" '.PaginaSEI::montarTitleTooltip('Ícone Ação Processo Anexado Acesso Externo Autorizado ABC','Módulo ABC').'><img src="modulos/abc/exemplo/imagens/abc_pequeno.png" class="imagemStatus" /></a>';
    }
    return $arrIcones;
  }
