...
class PenAtualizarSipRN extends InfraRN
{
  ...
  protected function atualizarVersaoConectado()
    ...
    try {
      ...
      if (
        ...
        //testando permissoes de criaÃ§ões de tabelas
  ...
  protected function getNumIdMenu($strMenu = 'Principal', $numIdSistema = 0)
     ...
     * @return int módigo do Recurso gerado
     */
  ...
  public function atribuirPerfil($numIdSistema)
    ...
    /**
     * Atualiza o nÃƒÂºmero de versão do módulo nas tabelas de parâmetro do sistema
     *
  ...
  private function atualizarNumeroVersao($parStrNumeroVersao)
    ...
    /**
     * Obtém id do item de menu, baseado no sistema, mótulo e id do item superior
     *
  ...
  private function instalarV101()
    ...
    if (empty($objItemMenuDTO)) {
      ...
      // Administrao > Mapeamento de Himóteses Legais de Envio
      $numIdItemMenu = $this->criarMenu('Mapeamento de Himóteses Legais', 20, $objItemMenuDTO->getNumIdItemMenu(), $numIdMenu, null, $numIdSistema);

      // Administrao > Mapeamento de Himóteses Legais de Envio > Envio
      $numIdItemMenu = $this->criarMenu('Envio', 10, $numIdItemMenu, $numIdMenu, null, $numIdSistema);

      // Administrao > Mapeamento de Himóteses Legais de Envio > Envio > Cadastrar
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_enviado_alterar', 'Alterar de mapeamento de Himóteses Legais de Envio', $numIdSistema);
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_enviado_cadastrar', 'Cadastro de mapeamento de Himóteses Legais de Envio', $numIdSistema);
      $this->criarMenu('Cadastrar', 10, $numIdItemMenu, $numIdMenu, $numIdRecurso, $numIdSistema);

      // Administrao > Mapeamento de Himóteses Legais de Envio > Envio > Listar
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_enviado_excluir', 'Excluir mapeamento de Himóteses Legais de Envio', $numIdSistema);
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_enviado_listar', 'Listagem de mapeamento de Himóteses Legais de Envio', $numIdSistema);
      ...
      $objDTO->setNumIdSistema($numIdSistema);
      $objDTO->setNumIdMenu($numIdMenu);
      $objDTO->setStrRotulo('Mapeamento de Himóteses Legais');
      $objDTO->setNumMaxRegistrosRetorno(1);
      $objDTO->retNumIdItemMenu();
    ...
    if (empty($objDTO)) {
      ...
      // Administrao > Mapeamento de Himóteses Legais de Envio > Envio > Cadastrar
      $this->criarRecurso('pen_map_hipotese_legal_recebido_alterar', 'Alteração de mapeamento de Himóteses Legais de Recebimento', $numIdSistema);
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_recebido_cadastrar', 'Cadastro de mapeamento de Himóteses Legais de Recebimento', $numIdSistema);
      $this->criarMenu('Cadastrar', 10, $numIdItemMenu, $numIdMenu, $numIdRecurso, $numIdSistema);

      // Administrao > Mapeamento de Himóteses Legais de Envio > Envio > Listar
      $this->criarRecurso('pen_map_hipotese_legal_recebido_excluir', 'Exclusão de mapeamento de Himóteses Legais de Recebimento', $numIdSistema);
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_recebido_listar', 'Listagem de mapeamento de Himóteses Legais de Recebimento', $numIdSistema);
      $this->criarMenu('Listar', 20, $numIdItemMenu, $numIdMenu, $numIdRecurso, $numIdSistema);
    ...
    if (empty($objDTO)) {
      ...
      $numIdRecurso = $this->criarRecurso('pen_map_hipotese_legal_padrao_cadastrar', 'Acesso ao formulário de cadastro de mapeamento de Himóteses Legais Padrão', $numIdSistema);

      $this->criarMenu('Himótese de Restrição Padrão', 30, $objDTO->getNumIdItemMenu(), $numIdMenu, $numIdRecurso, $numIdSistema);
      $this->criarRecurso('pen_map_hipotese_legal_padrao', 'Método Cadastrar Padrão da RN de mapeamento de Himóteses Legais', $numIdSistema);
      $this->atribuirPerfil($numIdSistema);
  ...
  private function instalarV103()
    {
      ...
      $objDTO = $objBD->consultar($objDTO);
    if ($objDTO) {
        $objDTO->setStrRotulo('Himótese de Restrição Padrão');
        $objBD->alterar($objDTO);
    }
  ...
  private function instalarV2000_beta1()
    ...
    try {
      ...
      $arrOrdemMenusAdministracaoPEN = array(
          ...
          array("rotulo" => "Mapeamento de Himóteses Legais",    "sequencia" => 40, "rotuloMenuSuperior" => "Processo Eletrônico Nacional"),
      );
      ...
      array_map(function ($item) use ($numIdSistemaSei, $numIdMenuSEI) {
          ...
          // Obtém id do item de menu, baseado no sistema, mótulo e, principalmente, ID DO ITEM SUPERIOR
  ...
  protected function instalarV2101()
    {
      // Adição de recursos relacionados à consulta de mapeamento de himóteses legais de envio e recebimento
      $numIdSistemaSei = $this->getNumIdSistema('SEI');
      $numIdPerfilSeiAdministrador = ScriptSip::obterIdPerfil($numIdSistemaSei, "Administrador");
      $this->criarRecurso('pen_map_hipotese_legal_recebimento_consultar', 'Consulta de mapeamento de Himóteses Legais de Recebimento', $numIdSistemaSei);
      ...
      $this->criarRecurso('pen_map_hipotese_legal_envio_consultar', 'Consulta de mapeamento de Himóteses Legais de Envio', $numIdSistemaSei);
  ...
  protected function instalarV30113()
    {
      ...
      // A partir da versão 3.0.0 é que o SIP passa a dar suporte à í­cones
  ...
  protected function instalarV3024()
    ...
    protected function instalarV3033()
    {
      ...
      //Cadastrar o perfil
      $id_perfil_tramitador = $fnCadastrarPerfil($numIdSistema, 'Tramitador de Processos em Bloco', 'Acesso aos recursos especÃ­Â­ficos ao perfil Tramitador de Processos em Bloco', 'N', 'S');
    ...
    // adicionar permissão
    $numIdPerfilSeiBasico = ScriptSip::obterIdPerfil($numIdSistema, "Básico");
    ...
    $this->criarRecurso('pen_tramite_em_bloco_consultar ', 'Cancelar processos do bloco de Tramite Externo', $numIdSistema);

    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'md_pen_tramita_em_bloco');
    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_tramite_em_bloco_cadastrar');
    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_tramite_em_bloco_alterar');
    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_tramite_em_bloco_consultar');
    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_tramite_em_bloco_cancelar');
    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_tramita_em_bloco_protocolo_listar');
    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_tramita_em_bloco_protocolo_excluir');

    ScriptSip::adicionarRecursoPerfil($numIdSistema, $numIdPerfilSeiBasico, 'pen_incluir_processo_em_bloco_tramite');
