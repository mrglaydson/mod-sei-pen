...
try {
    ...
    switch ($_GET['acao']) {
        ...
        case 'gd_prep_list_eliminacao_listar':
            SessaoSEI::getInstance()->validarPermissao('gestao_documental_prep_list_eliminacao_listar');
    ...
    $bolAcaoGerar = SessaoSEI::getInstance()->verificarPermissao('gestao_documental_prep_list_eliminacao_gerar');
    $bolAcaoExcluir = SessaoSEI::getInstance()->verificarPermissao('gestao_documental_prep_list_eliminacao_excluir');
    $bolAcaoObservar = SessaoSEI::getInstance()->verificarPermissao('gestao_documental_prep_list_eliminacao_observar');
    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            if ($bolAcaoObservar) {
                $strLinkObservar = SessaoSEI::getInstance()->assinarLink('controlador.php?acao=gd_prep_list_eliminacao_observar&acao_origem=' . $_GET['acao'] . '&id_arquivamento=' . $arrObjMdGdArquivamentoDTO[$i]->getNumIdArquivamento());
